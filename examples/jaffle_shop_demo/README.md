...
datalex datalex dbt sync . --out-root datalex-out # syncs into DataLex YAML
...
2. `datalex datalex dbt sync` read `target/manifest.json`, resolved the `dev`
...
- Emit back to dbt-parseable YAML with contracts enforced:
  ```bash
  datalex datalex dbt emit datalex-out --out-dir dbt-out
  ```
...
- Run DDL generation for a different dialect:
  ```bash
  datalex datalex emit ddl datalex-out --dialect snowflake
  ```
