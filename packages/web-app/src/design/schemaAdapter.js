...
function parseEndpoint(value) {
  ...
  if (!s.includes(".")) return { table: s.toLowerCase(), col: undefined };
...
function parseRelationshipSide(value) {
  ...
  if (value && typeof value === "object") {
    ...
    const rawCol = value.col || value.field || value.column;
    const col = rawCol ? String(rawCol).toLowerCase() : undefined;
    return { table, col };
  }
  return { table: "", col: undefined };
}
...
export function adaptDataLexYaml(yamlText) {
  ...
  const tables = entities.map((e, i) => {
    ...
    return {
      ...
      interface: e?.interface && typeof e.interface === "object" ? e.interface : undefined,
      ...
      badges: [
        ...(kind === "ENUM" ? ["ENUM"] : ["BASE"]),
        ...(e?.interface?.enabled || ["shared", "contracted"].includes(String(e?.interface?.stability || "").toLowerCase()) ? ["INTERFACE"] : []),
      ],
...
function dataLexColumnToField(c) {
  ...
  if (c.foreign_key && typeof c.foreign_key === "object") out.foreign_key = c.foreign_key;
    ...
    out.foreign_key = {
      entity: c.references.entity || c.references.table || c.references.references,
      field: c.references.field || c.references.column,
    };
  }
...
function dataLexModelDocToEntity(doc) {
  ...
  const entity = {
    name,
    type: layer === "conceptual" ? "concept" : (layer === "logical" ? "logical_entity" : String(doc?.type || "table")),
...
export function adaptDiagramYaml(yamlText, projectFiles) {
  ...
  inlineEntries.forEach((entry, index) => {
    ...
    const table = {
      ...
      interface: entry?.interface && typeof entry.interface === "object" ? entry.interface : undefined,
      ...
      badges: [
        layer === "logical" ? "LOGICAL" : layer === "physical" ? "DBT" : "BUSINESS",
        ...(entry?.interface?.enabled || ["shared", "contracted"].includes(String(entry?.interface?.stability || "").toLowerCase()) ? ["INTERFACE"] : []),
      ],
      rowCount: "",
      columns: cols,
      _inlineDiagram: true,
