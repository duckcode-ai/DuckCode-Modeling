# `datalex datalex` CLI cheat sheet
...
|---|---|
| `datalex datalex dbt sync <project> --out-root <dir>` | Reads `target/manifest.json` + profiles.yml, introspects live warehouse columns, merges into DataLex YAML. Idempotent: user edits survive re-sync. |
| `datalex datalex dbt emit <root> --out-dir <dir>` | Emits dbt `sources.yml` + `models/_schema.yml` with `contract.enforced: true` and `data_type:` per column. |
| `datalex datalex dbt import <manifest.json> --out-root <dir>` | One-shot import without warehouse introspection (manifest `data_type` only). |
...
| Command | What it does |
|---|---|
| `datalex datalex validate <root>` | Load the project, run schema + policy validation, exit non-zero on error. |
| `datalex datalex info <root>` | Entity-by-layer counts, physical-by-dialect breakdown, term/domain/policy counts. |
| `datalex datalex expand <root>` | Preview a project with snippets inlined (doesn't mutate files). |
...
| Command | What it does |
|---|---|
| `datalex datalex diff <old-root> <new-root>` | Detect added / removed / renamed / changed entities and columns. |
...
| Command | What it does |
|---|---|
| `datalex datalex packages resolve <root>` | Resolve `imports:` in `datalex.yaml`, fetch packages, write `.datalex/lock.yaml`. |
| `datalex datalex packages list <root>` | Show resolved packages and their cached paths. |
...
| Command | What it does |
|---|---|
| `datalex datalex migrate to-datalex-layout <legacy.model.yaml>` | Explode a v1/v2 `*.model.yaml` into the per-`kind:` DataLex tree. |
...
./datalex datalex diff datalex/ ../new/datalex/ --exit-on-breaking
```

**Round-trip sanity check — dbt sync, emit, parse:**
```bash
./datalex datalex dbt sync my-dbt-project --out-root datalex/
./datalex datalex dbt emit datalex/ --out-dir dbt-emitted/
...
```bash
./datalex datalex dbt sync my-dbt-project --out-root datalex/ --skip-warehouse
```

**Inspect a project without opening every file:**
```bash
./datalex datalex info datalex/ --output-json | jq .
```
